...
export class TicketDetailComponent implements OnInit, AfterViewChecked {
  ...
  ngOnInit() {
    ...
    if (this.JsonApiService.SFConfig.showCloseTicket) {
      this.showCloseTickets = true;
    }
  ...
  CloseTicket(){
    ...
    this.JsonApiService.UpdateRecFun(this.ApiKey, 'Ticket', this.ticket_data.TicketID, data, 'Close',this.ticket_data.EndUser).subscribe(
      () => {
