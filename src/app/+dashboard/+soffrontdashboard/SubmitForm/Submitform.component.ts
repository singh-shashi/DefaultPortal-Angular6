            ...
            for (let index = 0; index < this.FieldData.length; index++) {
              if (this.FieldData[index].ISEXTERNAL === 'Y' && this.FieldData[index].DEPENDFLDID === 0) {
                ...
                const fldValues =  this.jsonService._getFieldValue(this.ApiKey, this.FieldData[index].FORMNAME,
                  ...
                  .subscribe((fldVals) => {
                    if(fldVals.Object !== undefined && fldVals.Object !==null){
                      let con="Status='Active'"
                    if(this.JsonApiService.SFConfig.soffrontapi.ProjectName.toString().toUpperCase()=='PIOTECH' 
                       && this.FieldData[index].FieldName.toString().toUpperCase()=='P'){
                        
                        const allProduct= this.jsonService.SearchByPost(this.ApiKey,'NProduct','PName',0,0,'','',con)
                                          .subscribe((allProductDs) => {

                                            if(allProductDs.Object.RESULTS.length>0)
                                            {
                                              const accProductData = this.jsonService.SearchByLinkRelated(this.ApiKey, 'Account', this.LoggedInUser.selectedAccount.AcctID, 'NProduct'
                                              , '', 0, 0, '', con) .subscribe((accProduct) => {
                                              if(accProduct.Object.RESULTS!=undefined && accProduct.Object.RESULTS.length>0)
                                              {        
                                              //  let arr = fldVals.Object.filter(function(e){ return e.replace(/(\r\n|\n|\r)/gm,"")});               
                                              //   arrproduct = arr.join(',').split(',');
                                              accProduct.Object.RESULTS.forEach(element => {
                                                //const v =  accProduct.Object.RESULTS.filter(f=>f.FormId==element.FormId).FormId;
                                                if(allProductDs.Object.RESULTS.filter(f=>f.FormId==element.FormId).length>0)
                                                {
                                                  arrpval.push(element.PName);
                                                }
                                                });
                                                this.FieldData[index].FieldValues=arrpval.join(',');
                                              }
                                               accProductData.unsubscribe();
